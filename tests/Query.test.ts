...
describe('Query', () => {
  ...
  it('filter parameters should nest under filter key', () => {
    let filters = [new FilterSpec('name', 'Bob'), new FilterSpec('age', '99')]
    ...
    assert.equal(
      query.toString(),
      model.getJsonApiType() + '?filter%5Bname%5D=Bob&filter%5Bage%5D=99'
    )
  })
  ...
  it('include parameters should nest under include key', () => {
    ...
    assert.equal(
      query.toString(),
      model.getJsonApiType() + '?include=weapons%2Ccostume'
    )
  })

  it('option parameters should append to the query string', () => {
    let options = [new Option('name', 'Bob'), new Option('age', '99')]
  ...
  it('sort parameters should append to sort key', () => {
    let sorts = [new SortSpec('name', true), new SortSpec('age', false)]
